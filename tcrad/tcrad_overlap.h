...
subroutine calc_overlap_matrices_nocol_dp(nlev, is_clear_sky_layer, &
  ...
  ! Output overlap matrices
  real(jprb), intent(out), dimension(NREGION,NREGION,nlev+1) :: v_matrix, u_matrix
  ! real(jprb), intent(out), optional, dimension(NREGION,NREGION,nlev+1) &
  !       &  :: u_matrix
  ...
  ! Overlap matrix (non-directional)
  real(jprd) :: overlap_matrix(NREGION,NREGION)

  ! Fraction of the gridbox occupied by each region in the upper and
  ! lower layers for an interface
  real(jprd) :: frac_upper(NREGION), frac_lower(NREGION)

  ! Beta overlap parameter for each region
  real(jprd) :: op(NREGION)
  ...
  do jlev = 1,nlev+1
    ...
    if (is_clear_sky_layer(jlev) .and. is_clear_sky_layer(jlev-1)) then 
      ...
      if (jlev == 1 .or. jlev > nlev) then
        ...
        if (op(1) >= 0.0_jprd) then
          if (used_decorrelation_scaling==0.5_jprb) then 
            op(2:NREGION) = op(1)*op(1)
          else
            op(2:NREGION) = op(1)**(1.0_jprd/used_decorrelation_scaling)
          end if
        else
          op(2:NREGION) = op(1)
        end if
      ...
      !   overlap_matrix = calc_beta_overlap_matrix_dp(NREGION, op, &
      !       &  frac_upper, frac_lower, frac_threshold)
      ! else
      ! Simpler scheme assuming the two cloudy regions have the
      ! same fraction
      !overlap_matrix = calc_alpha_overlap_matrix_simple(NREGION, &
      !     &  op(1), op(2), &
      !     &  1.0_jprd - frac_upper(1), 1.0_jprd - frac_lower(1))
      ! More general scheme
      overlap_matrix = calc_alpha_overlap_matrix_dp(NREGION, &
          &  op(1), op(2), frac_upper, frac_lower)
      ! end if
 ...
 pure function calc_alpha_overlap_matrix_dp(nreg, op, op_inhom, &
    ...
    ! Fraction of the gridbox occupied by each region in the upper and
    ! lower layers
    real(jprd), intent(in), dimension(NREGION) :: frac_upper, frac_lower
